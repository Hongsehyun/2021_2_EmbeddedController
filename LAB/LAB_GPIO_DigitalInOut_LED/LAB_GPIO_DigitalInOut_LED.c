...
*/
	...
	GPIO_output(GPIOB, 8, Medium, PushPull , PU);
	GPIO_output(GPIOB, 9, Medium, PushPull , PU);
...
void setup(void){
	...
	GPIO_init(GPIOB, 8, OUTPUT);    // calls RCC_GPIOA_enable()
	GPIO_init(GPIOB, 9, OUTPUT);    // calls RCC_GPIOB_enable()
}
