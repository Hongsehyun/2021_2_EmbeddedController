...
// Configuration setting and interrupt enable for USART execution.
void USART_begin(USART_TypeDef* USARTx, GPIO_TypeDef* GPIO_TX, int pinTX, GPIO_TypeDef* GPIO_RX, int pinRX, int baud){
  ...
  GPIO_pudr(GPIO_TX, pinTX, N_PUPD);
	GPIO_pudr(GPIO_RX, pinRX, N_PUPD);
  ...
  // 2. USARTx (x=2,1,6) configuration	
	...
	//uint16_t USARTDIV = Hz/baud ;
	//USARTx->BRR  |= USARTDIV;
	USARTx->CR3 |= USART_CR3_DMAT | USART_CR3_DMAR;

	// Enable TX, RX, and USARTx 
	USARTx->CR1  |= USART_CR1_TE;   	// Transmitter and Receiver enable
	USARTx->CR1  |= USART_CR1_RE;   	// Transmitter and Receiver enable
	
  // 3. Read USARTx Data (Interrupt)	
	// Set the priority and enable interrupt
	USARTx->CR1 |= USART_CR1_RXNEIE;            // Received Data Ready to be Read Interrupt
	if (USARTx==USART1){
		NVIC_SetPriority(USART1_IRQn, 9);      		// Set Priority to 1
		NVIC_EnableIRQ(USART1_IRQn);    				  // Enable interrupt of USART1 
	}
	else if (USARTx==USART2){
		NVIC_SetPriority(USART2_IRQn, 8);      		// Set Priority to 1
		NVIC_EnableIRQ(USART2_IRQn);             	// Enable interrupt of USART2 peripheral
	}
	else {																			// if(USARTx==USART6)
		NVIC_SetPriority(USART6_IRQn, 7);      		// Set Priority to 1
...
// Matching Port and Pin to setted USART Num.
void USART_init(USART_TypeDef* USARTx, int baud){
	// Using when know USART Number. Instead of using port and pin Num.
	// Define Default
	  // **********************************************************
...
// Wait until RXNE bit is set by HW. When RXNE is set to 1, this means Read to read
